  ...
  return this.each(function () {
    ...
    this.dragCfg = {
      ...
      callback:
        cfg && cfg.callback
          ? cfg.callback
          : () => console.log("cb does not exist"),
    };

    // if a smallStep isn't supplied, use half the regular step
    this.dragCfg.smallStep =
      cfg && cfg.smallStep ? cfg.smallStep : this.dragCfg.step / 2;
    ...
    if ($cursor && !isNaN(this.dragCfg.start))
      $cursor.css("top", (this.dragCfg.start * -1) / scale + cursorHeight);
    ...
    this.launch = function (e) {
      ...
      var val = this.value;
      var el = this;
      if (attr && selectedElems.length)
        svgCanvas.undoMgr.beginUndoableChange(attr, selectedElems);
      $("body").addClass("dragging");
      $label.addClass("active");
      $(window).bind("mousemove.draginput touchmove.draginput", function (e) {
        el.move(e, oy, parseFloat(val));
      });
      $(window).bind("mouseup.draginput touchend.draginput", function (e) {
        el.stop();
      });
      $("input").blur();
    };
    ...
    $(this)
      ...
      .bind("mousewheel", function (e, delta, deltaX, deltaY) {
        ...
        if (deltaY > 0) this.adjustValue(this.dragCfg.step);
        else if (deltaY < 0) this.adjustValue(-this.dragCfg.step);
        e.preventDefault();
      });
  });
};
