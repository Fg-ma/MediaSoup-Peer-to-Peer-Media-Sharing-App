MD.Zoom = function () {
  ...
  $("#workarea").on("mousewheel", function (e, delta, deltaX, deltaY) {
    if (e.altKey || e.ctrlKey) {
      ...
      $zoom
        .val(Math.max(0, parseInt(zoom + deltaY * (e.altKey ? 50 : 20))))
        .change();
    }
  });
  ...
  function changed(window, bbox) {
    ...
    const z_info = svgCanvas.setBBoxZoom(
      bbox,
      $workarea.width() - scrbar,
      $workarea.height() - scrbar
    );
