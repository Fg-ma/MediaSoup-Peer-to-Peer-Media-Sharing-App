...
(function () {
  ...
  svgedit.draw.randomizeIds = function (enableRandomization, current_drawing) {
    ...
    if (
      randomize_ids === RandomizeModes.ALWAYS_RANDOMIZE &&
      !current_drawing.getNonce()
    ) {
      current_drawing.setNonce(Math.floor(Math.random() * 100001));
    } else if (
      randomize_ids === RandomizeModes.NEVER_RANDOMIZE &&
      current_drawing.getNonce()
    ) {
   ...
   * Gets the idPrefix
   */
  svgedit.draw.Drawing.prototype.getIdPrefix = function (prefix) {
  ...
  svgedit.draw.Drawing.prototype.releaseId = function (id) {
    ...
    if (
      typeof num != typeof 1 ||
      num <= 0 ||
      this.releasedNums.indexOf(num) != -1
    ) {
      return false;
    }
  ...
  svgedit.draw.Drawing.prototype.setLayerVisibility = function (
    layername,
    bVisible
  ) {
  ...
  svgedit.draw.Drawing.prototype.setLayerOpacity = function (
    layername,
    opacity
  ) {
