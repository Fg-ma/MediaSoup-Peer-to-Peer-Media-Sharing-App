...
(function () {
  ...
  svgedit.history.MoveElementCommand = function (
    elem,
    oldNextSibling,
    oldParent,
    text
  ...
  svgedit.history.MoveElementCommand.prototype.apply = function (handler) {
    ...
    if (handler) {
      handler.handleHistoryEvent(
        svgedit.history.HistoryEventTypes.AFTER_APPLY,
        this
      );
    }
  };
  ...
  // text - An optional string visible to user related to this change
  svgedit.history.RemoveElementCommand = function (
    elem,
    oldNextSibling,
    oldParent,
    text
    ...
    while (i--) {
      ...
      if (changes[attrName] != elem.getAttribute(attrName)) {
        batchCmd.addSubCommand(
          new svgedit.history.ChangeElementCommand(elem, changes, attrName)
        );
      }
    }
