...
(function () {
  ...
  svgedit.utilities.getHref = function (elem) {
    if (elem)
      return elem.getAttributeNS(XLINKNS, "href") || elem.getAttribute("href");
  };
  ...
  svgedit.utilities.findDefs = function (svgElement) {
    ...
    if (defs.length > 0) {
      ...
      defs = svgElement.insertBefore(
        svgElement.ownerDocument.createElementNS(SVGNS, "defs"),
        svgElement.firstChild.nextSibling
      );
    }
    return defs;
  };
  ...
  svgedit.utilities.assignAttributes = function (
    node,
    attrs,
    suspendLength,
    unitCheck
