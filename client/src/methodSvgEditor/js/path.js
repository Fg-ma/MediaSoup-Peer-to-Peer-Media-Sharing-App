...
(function () {
  ...
  svgedit.path.addPointGrip = function (index, x, y) {
    ...
    if (!pointGrip) {
      ...
      svgedit.utilities.assignAttributes(pointGrip, {
        ...
        "xlink:title":
          "Drag node to move it. Double-click node to change segment type",
      });
  ...
  svgedit.path.smoothControlPoints = this.smoothControlPoints = function (
    ...
    if ((x1 != 0 || y1 != 0) && (x2 != 0 || y2 != 0)) {
      ...
      if (anglea - angleb > 0) {
        new_anglea =
          angleBetween < Math.PI ? anglea + angleDiff : anglea - angleDiff;
        new_angleb =
          angleBetween < Math.PI ? angleb - angleDiff : angleb + angleDiff;
      } else {
        new_anglea =
          angleBetween < Math.PI ? anglea - angleDiff : anglea + angleDiff;
        new_angleb =
  ...
  svgedit.path.Segment.prototype.select = function (y) {
    if (this.ptgrip) {
      this.ptgrip.setAttribute("stroke", "#d40213");
      this.ptgrip.setAttribute("fill", y ? "#d40213" : "#f2f2f2");
      this.segsel.setAttribute("display", y ? "inline" : "none");
      if (this.ctrlpts) {
        this.selectCtrls(y);
      }
