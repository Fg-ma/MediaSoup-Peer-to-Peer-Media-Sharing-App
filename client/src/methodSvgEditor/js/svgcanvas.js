...
$.SvgCanvas = function (container, config) {
  ...
  var getTransformList = (canvas.getTransformList =
    svgedit.transformlist.getTransformList);

  // import from math.js.
  var transformPoint = svgedit.math.transformPoint;
  var matrixMultiply = (canvas.matrixMultiply = svgedit.math.matrixMultiply);
  var hasMatrixTransform = (canvas.hasMatrixTransform =
  ...
  var getUrlFromAttr = (canvas.getUrlFromAttr =
  ...
  var getRotationAngle = (canvas.getRotationAngle =
    svgedit.utilities.getRotationAngle);
  var getElem = (canvas.getElem = svgedit.utilities.getElem);
  var assignAttributes = (canvas.assignAttributes =
    svgedit.utilities.assignAttributes);
    ...
    handleHistoryEvent: function (eventType, cmd) {
      ...
      if (
        ...
        if (cmdType == MoveElementCommand.type()) {
          ...
          canvas.setOffsets(cmd.elem.parentNode);
  ...
  var selectorManager = (this.selectorManager =
    svgedit.select.getSelectorManager());
  // this object manages selectors for us
  ...
  this.addExtension = function (name, ext_func) {
    if (!(name in extensions)) {
      ...
      if ($.isFunction(ext_func)) {
        var ext = ext_func(
          $.extend(canvas.getPrivateMethods(), {
            svgroot: svgroot,
  ...
  var getVisibleElementsAndBBoxes = (this.getVisibleElementsAndBBoxes =
    function (parent) {
      if (!parent) parent = $(svgcontent).children(); // Prevent layers from being included
  ...
  var remapElement = (this.remapElement = function (selected, changes, m) {
    var remap = function (x, y) {
      ...
      finishUp = function () {
        if (doSnapping)
          for (var o in changes) changes[o] = snapToGrid(changes[o]);
    ...
    switch (elName) {
      ...
      case "circle":
        ...
        var tbox = svgedit.math.transformBox(
          box.x,
          box.y,
          box.width,
          box.height,
  ...
  var recalculateDimensions = (this.recalculateDimensions = function (
    ...
    if (tlist) {
      ...
      if (
        k >= 2 &&
        tlist.getItem(k - 2).type === 1 &&
        tlist.getItem(k - 1).type === 2
      ) {
        var mt = svgroot.createSVGTransform();
      ...
      default:
        if (
          ...
          (tlist.numberOfItems === 2 &&
            tlist.getItem(0).type === 1 &&
            tlist.getItem(0).type === 4)
        ) {
          return null;
        }
    }
    ...
    if ((selected.tagName == "g" && !gsvg) || selected.tagName == "a") {
      ...
      // therefore [ T2 ] = [ M_inv ] [ T1 ] [ M ]
        ...
        (N == 1 || (N > 1 && tlist.getItem(1).type != 3)) &&
  ...
  var removeFromSelection = (this.removeFromSelection = function (
    ...
    (j = 0), (len = selectedElements.length);
    for (var i = 0; i < len; ++i) {
      var elem = selectedElements[i];
      if (elem) {
        // keep the item
  ...
  var getMouseTarget = (this.getMouseTarget = function (evt) {
    ...
    if (
      [mathns, htmlns].indexOf(mouse_target.namespaceURI) >= 0 &&
    ...
    var current_layer = getCurrentDrawing().getCurrentLayer();
    if (
      [svgroot, container, svgcontent, current_layer].indexOf(mouse_target) >= 0
    ) {
      return svgroot;
    }
  ...
  (function () {
    ...
    var mouseDown = function (evt) {
      ...
      if (
        ...
        mouse_target.childNodes.length === 1
      ) {
      ...
      switch (current_mode) {
        case "select":
          ...
          if (selectedElements[0] !== null) {
            ...
            if (evt.shiftKey) {
              var xya = snapToAngle(start_x, start_y, x, y);
              x = xya.x;
              y = xya.y;
            }
            if (dx != 0 || dy != 0) {
        ...
        case "multiselect":
          ...
          if (elemsToAdd.length > 0) addToSelection(elemsToAdd);
        ...
        case "resize":
          ...
          if (
            current_resize_mode.indexOf("n") == -1 &&
            current_resize_mode.indexOf("s") == -1
          ) {
            dy = 0;
          }
          if (
            current_resize_mode.indexOf("e") == -1 &&
            current_resize_mode.indexOf("w") == -1
          ) {
            dx = 0;
          }
        ...
        case "line":
          ...
          if (evt.shiftKey) {
            var xya = snapToAngle(start_x, start_y, x2, y2);
            x2 = xya.x;
            y2 = xya.y;
          }
        ...
        case "rotate":
          ...
          if (current_rotate_mode == "sw") {
            ccx = box.x + box.width;
            ccy = box.y + box.height;
          }
          compensation_angle =
            (Math.atan2(cy - ccy, cx - ccx) * (180 / Math.PI) - 90) % 360;
  ...
  */
    ...
    var mouseUp = function (evt) {
      ...
      switch (current_mode) {
        ...
        case "image":
          ...
          keep =
            attrs.width != 0 || attrs.height != 0 || current_mode === "image";
          break;
        case "circle":
  ...
  var pathActions = (canvas.pathActions = (function () {
    ...
    return {
          ...
          if (!drawn_path) {
            ...
            if (clickOnPoint) {
              ...
              if (i <= 1 && len >= 2) {
                ...
                var grip_x = svgedit.path.first_grip
                  ? svgedit.path.first_grip[0] / current_zoom
                  : seglist.getItem(0).x;
                var grip_y = svgedit.path.first_grip
                  ? svgedit.path.first_grip[1] / current_zoom
                  : seglist.getItem(0).y;
      ...
      mouseMove: function (evt, mouse_x, mouse_y) {
        ...
        if (current_mode === "path") {
          ...
          if (newPoint) {
            ...
            // Set control points
            var current_pointGrip2_x =
              pointGrip2.getAttribute("cx") / current_zoom || 0;
            var current_pointGrip2_y =
              pointGrip2.getAttribute("cy") / current_zoom || 0;
            ...
            var alt_x = is_linked
              ? pt_x + (pt_x - cur_x)
              : current_pointGrip2_x;
            var alt_y = is_linked
              ? pt_y + (pt_y - cur_y)
              : current_pointGrip2_y;
            ...
            if (index === 0) {
              ...
              svgedit.path.replacePathSeg(
                ...
                [
                  pt_x,
                  pt_y,
                  this.lastCtrlPoint[0] / current_zoom,
                  this.lastCtrlPoint[1] / current_zoom,
                  alt_x,
                  alt_y,
                ],
                drawn_path
            ...
            if (stretchy) {
              ...
              if (prev.pathSegType === 6) {
                var prev_x =
                  this.lastCtrlPoint[0] / current_zoom ||
                  prev.x + (prev.x - prev.x2);
                var prev_y =
                  this.lastCtrlPoint[1] / current_zoom ||
                  prev.y + (prev.y - prev.y2);
                svgedit.path.replacePathSeg(
                  6,
                  1,
                  [
        ...
        if (svgedit.path.path.dragging) {
          ...
          if (!is_linked || !svgedit.path.is_linked)
            svgedit.path.setLinkControlPoints(false);
      ...
      mouseUp: function (evt, element, mouse_x, mouse_y) {
        ...
        if (lastpointgrip)
          this.lastCtrlPoint = [
            lastpointgrip.getAttribute("cx"),
            lastpointgrip.getAttribute("cy"),
          ];
        else this.lastCtrlPoint = [mouse_x, mouse_y];
        if (!svgedit.path.first_grip) {
          if (firstpointgrip) {
            svgedit.path.first_grip = [
              firstpointgrip.getAttribute("cx"),
              firstpointgrip.getAttribute("cy"),
            ];
          } else {
      ...
      clear: function (remove) {
        ...
        if (drawn_path) {
          ...
          $(getElem("pathpointgrip_container"))
            .find("*")
            .attr("display", "none");
      ...
      getNodePoint: function () {
        ...
        var sel_pt = svgedit.path.path.selected_pts.length
      ...
      convertPath: function (path, toRel) {
        ...
        for (var i = 0; i < len; ++i) {
          ...
          var type = seg.pathSegType;
          var letter =
            pathMap[type]["to" + (toRel ? "Lower" : "Upper") + "Case"]();
          ...
          switch (type) {
            ...
            case 3: // relative move (m)
              ...
              if (
                last_m &&
                segList.getItem(i - 1).pathSegType === 1 &&
                !toRel
              ) {
                curx = last_m[0];
                cury = last_m[1];
  ...
  this.save = function () {
    ...
    if (illutratorCompat && str.includes(" href="))
    ...
    try {
      ...
      svgcontent.querySelectorAll("textPath").forEach(function (el) {
        ...
        if (offset.includes("%") && path) {
          ...
          const pct = parseFloat(offset) * 0.01;
    ...
    switch (p.type) {
      case "solidColor":
        if (p.solidColor != "none" && p.solidColor != "#none") {
          ...
          var selector =
            type == "fill" ? "#fill_color rect" : "#stroke_color rect";
  ...
  this.setPaintOpacity = function (type, val, preventUndo) {
    ...
    if (!preventUndo) changeSelectedAttribute(type + "-opacity", val);
    else changeSelectedAttributeNoUndo(type + "-opacity", val);
  };

  // Function: getEdgeDetection
  // Gets if there is edge detection or not
  //
  // Parameters:
  // elem - The element to check the edge dection existence
  this.getEdgeDetection = function (elem) {
    var val = false;

    if (elem) {
      var filter_url = elem.getAttribute("filter");
      if (filter_url) {
        var edgeDetection = getElem(elem.id + "_edge_detection");
        if (edgeDetection) val = true;
      }
    }
    return val;
  };
  ...
  (function () {
    ...
    // Sets the edgeDetection value on the selected element without being undoable
    ...
    // Function: setEffectNoUndo
    // Sets the effect value on the selected element without being undoable
    //
    // Parameters:
    ...
    canvas.setOffsets = function (filter) {
      assignAttributes(
        filter,
        {
          x: "-50%",
  ...
  this.setRectRadius = function (val) {
    if (
      canvas.elementsAreSame(selectedElements) &&
      selectedElements[0].tagName == "rect"
    ) {
  ...
  var changeSelectedAttributeNoUndo = (this.changeSelectedAttributeNoUndo =
    function (attr, newValue, elems) {
      if (current_mode == "pathedit") {
        // Editing node
      ...
      while (i--) {
        ...
        if (
          (attr === "x" || attr === "y") &&
          no_xy_elems.indexOf(elem.tagName) >= 0
        ) {
        ...
        var oldval =
          attr === "#text" ? elem.textContent : elem.getAttribute(attr);
  ...
  var pushGroupProperties = (this.pushGroupProperties = function (g, undoable) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      if (gattrs.filter) {
        ...
        var cgrayscale = this.getEffectAttr(
          elem,
          "_grayscale",
          "feColorMatrix",
          "values",
        ...
        var suffix =
          gfilter.firstChild.tagName === "feGaussianBlur" ? "blur" : "filter";
