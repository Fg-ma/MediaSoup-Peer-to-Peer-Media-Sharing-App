...
export default function GlassesButton({
  ...
  const streamEffects = userStreamEffects.current.video[videoId].video.glasses;
  const effectsStyles = userEffectsStyles.current.video[videoId].video.glasses;
  ...
  const holdFunction = async (event: PointerEvent) => {
    ...
    const effectType = target.dataset
      .videoEffectsButtonValue as GlassesEffectTypes;
    if (
            ...
            <FgButton
              ...
              contentFunction={() => (
                <div
                  ...
                  data-video-effects-button-value={glasses}
                >
                  <FgImageElement
                    src={effect.image}
